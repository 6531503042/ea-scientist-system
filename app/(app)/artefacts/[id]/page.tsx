...
export default function ArtefactDetailPage() {
  const { id } = useParams();
  const { getArtefactById, loading } = useArtefacts();
  const artefactId = Array.isArray(id) ? id[0] : id;

  const artefact = artefactId ? getArtefactById(artefactId) : null;

  if (loading) {
    return (
      <div className="p-6">
        <h1 className="text-2xl font-bold">Loading artefact...</h1>
      </div>
    );
  }
