...
import { EditTypeModal, type EditTypeTarget } from './_components/EditTypeModal';
...
export default function ArtefactsPage() {
    ...
    const { artefacts, loading: artefactsLoading, fetchArtefacts } = useArtefacts();
    const {
        layers,
        categories,
    ...
    const handleCreateCategory = async (data: Parameters<typeof createCategory>[0]) => {
    ...
    const handleUpdateLayer = async (id: number, data: Parameters<typeof updateLayer>[1]) => {
    ...
    const handleUpdateCategory = async (id: number, data: Parameters<typeof updateCategory>[1]) => {
    ...
    const handleDeleteLayer = async (id: number) => {
        if (confirm('คุณแน่ใจหรือไม่ว่าต้องการปิดใช้งาน Layer นี้?')) {
            ...
            toast(ok ? { title: 'ปิดใช้งาน Layer สำเร็จ' } : { variant: 'destructive', title: 'ปิดใช้งาน Layer ไม่สำเร็จ' });
        }
    };
    ...
    const handleDeleteCategory = async (id: number) => {
        if (confirm('คุณแน่ใจหรือไม่ว่าต้องการปิดใช้งาน Category นี้?')) {
            ...
            toast(ok ? { title: 'ปิดใช้งาน Category สำเร็จ' } : { variant: 'destructive', title: 'ปิดใช้งาน Category ไม่สำเร็จ' });
        }
    };
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-4 sm:px-6 pt-4 pb-2 border-b border-border shrink-0">
                ...
                {activeTab === 'types' && (
                    <Button size="sm" className="gap-1.5 h-8" onClick={() => setIsCreateTypeModalOpen(true)}>
