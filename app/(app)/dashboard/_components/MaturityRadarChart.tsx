...
export function MaturityRadarChart({
  ...
  const overallScore = Math.round(
    maturityData.reduce((sum, item) => sum + item.score, 0) /
      Math.max(maturityData.length, 1),
  );
    ...
    >
      <div className="mb-4">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          Based on artefact distribution
        </p>
        {/* Legend */}
...
export function QuickStats({
  ...
  const stats = [
    ...
    {
      label: "Coverage %",
      value: Math.round((activeCount / Math.max(totalArtefacts, 1)) * 100),
    },
  ];
