...
export function FloatingInsightPanel({
  ...
  const upstream = upstreamRels
    .map((r) => artefacts.find((a) => a.id === r.source))
    .filter(Boolean) as Artefact[];
  const downstream = downstreamRels
    .map((r) => artefacts.find((a) => a.id === r.target))
    .filter(Boolean) as Artefact[];
            ...
            <div className="relative py-4 mb-2">
              <div className="flex items-center justify-center gap-3">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-[9px] font-medium text-blue-500 mt-1">
                    Inputs
                  </span>
                </div>
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-[9px] font-medium text-amber-500 mt-1">
                    Outputs
                  </span>
                </div>
              </div>
                ...
                {impactUpstream.length > 0 ? (
                  ...
                  <div className="py-6 text-center">
                    ...
                    <p className="text-[10px] text-muted-foreground/60 mt-0.5">
                      {language === "th"
                        ? "ระบบนี้ไม่ขึ้นกับ input จากที่อื่น"
                        : "This system does not depend on external inputs"}
                    </p>
                  </div>
                )}
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-amber-100">
                      {t("detail.systemsReceivingData")} {artefact.name}
                    </p>
                  </div>
                </div>
                <span className="px-2 py-0.5 text-xs font-bold text-amber-600 bg-white rounded-full">
                  {impactDownstream.length}
                </span>
              </div>
              ...
              <div className="bg-amber-50/50 dark:bg-amber-950/20 p-2">
                {impactDownstream.length > 0 ? (
                  ...
                  <div className="py-6 text-center">
                    ...
                    <p className="text-[10px] text-muted-foreground/60 mt-0.5">
                      {language === "th"
                        ? "ไม่มีระบบอื่นที่ขึ้นกับ output นี้"
                        : "No other systems depend on this output"}
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="border-t pt-3">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() =>
                    setSimulationAction(
                      simulationAction === "modify" ? "none" : "modify",
                    )
                  }
                  className={cn(
                ...
                <button
                  onClick={() =>
                    setSimulationAction(
                      simulationAction === "delete" ? "none" : "delete",
                    )
                  }
                  className={cn(
                    "flex-1 py-2 px-2 rounded-lg border-2 transition-all flex items-center justify-center gap-1 text-xs font-medium",
                ...
                >
                  {simulationAction === "delete" ? (
                    <p className="text-destructive">
                      ⚠️ {impactStats.affected} ระบบจะไม่ทำงาน
                    </p>
                  ) : (
                    <p className="text-warning">
                      📋 ต้องทดสอบ {impactStats.affected} ระบบ
                    </p>
                  )}
                </motion.div>
              )}
              ...
              {upstream.length > 0 ? (
                <div className="space-y-1">
                  {upstream.slice(0, 4).map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center gap-2 p-2 bg-blue-500/5 border border-blue-500/10 rounded-lg"
                    >
                      <div
                        className={cn(
                          "w-2 h-2 rounded-full",
                      ...
                      <span className="text-[9px] text-muted-foreground">
                        {ARTEFACT_TYPE_LABELS[item.type]?.th}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              {downstream.length > 0 ? (
                <div className="space-y-1">
                  {downstream.slice(0, 4).map((item) => (
                    <div
                      key={item.id}
                      className="flex items-center gap-2 p-2 bg-amber-500/5 border border-amber-500/10 rounded-lg"
                    >
                      <div
                        className={cn(
                          "w-2 h-2 rounded-full",
