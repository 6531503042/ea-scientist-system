...
// Re-export for backward compatibility if needed, but components should use types/ directly
export type { Artefact, Relationship, DashboardMetric, ArtefactVersion, RiskLevel };

// Audit Log Entry needs to be moved to types/audit.d.ts eventually, but defining here for now if not present
import type { AuditLog } from '@/types/audit';

// Temporary fallback for AuditLogEntry if not in global types yet (matching existing mock usage)
...
// Use global constant instead of local definition
export const typeLabels = ARTEFACT_TYPE_LABELS;
