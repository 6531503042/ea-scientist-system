...
async function loginWithFallbacks() {
  ...
  throw new Error(
    "[Auth] login failed for all credential candidates. Set SMOKE_AUTH_EMAIL and SMOKE_AUTH_PASSWORD to valid credentials.",
  );
}
...
async function testAuthModule() {
  ...
  assert(
    meRes.response.ok,
    `[Auth] me failed: ${meRes.json?.error || meRes.response.status}`,
  );
  assert(meRes.json?.success === true, "[Auth] me success flag is false");

  const logoutRes = await request("/api/v1/auth/logout", { method: "POST" });
  assert(
    logoutRes.response.ok,
    `[Auth] logout failed: ${logoutRes.json?.error || logoutRes.response.status}`,
  );
  assert(
    logoutRes.json?.success === true,
    "[Auth] logout success flag is false",
  );

  const meAfterLogoutRes = await request("/api/v1/auth/me", { method: "GET" });
  assert(
    meAfterLogoutRes.response.status === 401,
    `[Auth] expected me after logout = 401, got ${meAfterLogoutRes.response.status}`,
  );
  ...
  if (activeCredential) {
    ...
    assert(
      reloginRes.response.ok,
      "[Auth] relogin failed for IAM module tests",
    );
  }
...
async function ensureRoleAndDepartment() {
  ...
  assert(
    rolesRes.response.ok,
    `[IAM] cannot load roles: ${rolesRes.json?.error || rolesRes.response.status}`,
  );
  ...
  if (!Number.isInteger(roleId)) {
    ...
    const createRoleRes = await request("/api/v1/roles", {
      ...
      body: JSON.stringify({
        ...
        description: "smoke bootstrap role",
        permissionIds: [],
      }),
    });
    assert(
      createRoleRes.response.ok,
      `[IAM] bootstrap role create failed: ${createRoleRes.json?.error || createRoleRes.response.status}`,
    );
    roleId = createRoleRes.json?.data?.id;
  }
  assert(
    Number.isInteger(roleId),
    "[IAM] no role id available after bootstrap",
  );

  const departmentsRes = await request("/api/v1/departments", {
    method: "GET",
  });
  assert(
    departmentsRes.response.ok,
    `[IAM] cannot load departments: ${departmentsRes.json?.error || departmentsRes.response.status}`,
  );
  ...
  if (!Number.isInteger(departmentId)) {
    ...
    const createDepartmentRes = await request("/api/v1/departments", {
      method: "POST",
      body: JSON.stringify({
        shortName: bootstrapShortName,
        fullName: bootstrapFullName,
      }),
    });
    assert(
      createDepartmentRes.response.ok,
      `[IAM] bootstrap department create failed: ${createDepartmentRes.json?.error || createDepartmentRes.response.status}`,
    );
    departmentId = createDepartmentRes.json?.data?.id;
  }
  assert(
    Number.isInteger(departmentId),
    "[IAM] no department id available after bootstrap",
  );
...
async function testUsersModule() {
  ...
  assert(
    listRes.response.ok,
    `[Users] list failed: ${listRes.json?.error || listRes.response.status}`,
  );
  ...
  assert(
    createRes.response.ok,
    `[Users] create failed: ${createRes.json?.error || createRes.response.status}`,
  );
  ...
  assert(
    updateRes.response.ok,
    `[Users] update failed: ${updateRes.json?.error || updateRes.response.status}`,
  );

  const deleteRes = await request(`/api/v1/users/${userId}`, {
    method: "DELETE",
  });
  assert(
    deleteRes.response.ok,
    `[Users] delete failed: ${deleteRes.json?.error || deleteRes.response.status}`,
  );
...
async function testRolesModule() {
  ...
  assert(
    listRes.response.ok,
    `[Roles] list failed: ${listRes.json?.error || listRes.response.status}`,
  );
  ...
  assert(
    createRes.response.ok,
    `[Roles] create failed: ${createRes.json?.error || createRes.response.status}`,
  );
  ...
  assert(
    updateRes.response.ok,
    `[Roles] update failed: ${updateRes.json?.error || updateRes.response.status}`,
  );

  const deleteRes = await request(`/api/v1/roles/${roleId}`, {
    method: "DELETE",
  });
  assert(
    deleteRes.response.ok,
    `[Roles] delete failed: ${deleteRes.json?.error || deleteRes.response.status}`,
  );
...
async function testDepartmentsModule() {
  ...
  assert(
    listRes.response.ok,
    `[Departments] list failed: ${listRes.json?.error || listRes.response.status}`,
  );
  assert(
    Array.isArray(listRes.json?.data),
    "[Departments] list data is not array",
  );
  ...
  assert(
    createRes.response.ok,
    `[Departments] create failed: ${createRes.json?.error || createRes.response.status}`,
  );
  const departmentId = createRes.json?.data?.id;
  assert(
    Number.isInteger(departmentId),
    "[Departments] create did not return department id",
  );
  ...
  assert(
    updateRes.response.ok,
    `[Departments] update failed: ${updateRes.json?.error || updateRes.response.status}`,
  );

  const deleteRes = await request(`/api/v1/departments/${departmentId}`, {
    method: "DELETE",
  });
  assert(
    deleteRes.response.ok,
    `[Departments] delete failed: ${deleteRes.json?.error || deleteRes.response.status}`,
  );
