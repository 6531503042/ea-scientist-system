...
function mapActiveStatus(status: unknown, fallbackIsActive?: boolean): 'active' | 'inactive' {
    const normalized = typeof status === 'string' ? status.toUpperCase() : undefined;
    if (normalized === 'ACTIVE') return 'active';
    if (normalized === 'INACTIVE' || normalized === 'LOCKED' || normalized === 'RETIRED') return 'inactive';
...
export function mapApiUser(apiUser: any): User {
    return {
        ...
        displayName: apiUser?.fullName || `${apiUser?.firstName ?? ''} ${apiUser?.lastName ?? ''}`.trim(),
        username: apiUser?.username || (apiUser?.email ? String(apiUser.email).split('@')[0] : ''),
        email: apiUser?.email ?? '',
        role: apiUser?.roleName || apiUser?.role?.roleName || apiUser?.role?.name || 'viewer',
        department: apiUser?.departmentName || apiUser?.department?.shortName || apiUser?.department?.fullName || undefined,
