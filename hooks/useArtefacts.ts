...
function transformApiArtefact(apiArtefact: any): Artefact {
  return {
    ...
    status:
      (apiArtefact.lifecycleStatus?.toLowerCase() as Artefact["status"]) ||
      "draft",
    ...
    lastUpdated:
      apiArtefact.updatedAt ||
      apiArtefact.createdAt ||
...
export function useArtefacts() {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      const apiData = data.artefactName
        ...
        : {
            ...(data.name && {
              artefactName: { en: data.name, th: data.nameTh || data.name },
            }),
            ...(data.description && {
  ...
  const getArtefactById = useCallback(
    (id: string) => {
  ...
  const getArtefactsByType = useCallback(
    (type: ArtefactType) => {
  ...
  return {
    ...
    updateArtefact: (id: string, data: Record<string, unknown>) =>
